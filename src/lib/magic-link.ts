...
export async function sendMagicLinkEmailWithUrl(email: string, magicUrl: string) {
  ...
  if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
    try {
        ...
        <div style="background: linear-gradient(135deg, #1a1a1a 0%, #2d1b69 100%); padding: 40px; font-family: system-ui, -apple-system, sans-serif;">
          <div style="max-width: 600px; margin: 0 auto; background: rgba(255,255,255,0.05); border-radius: 20px; padding: 40px; text-align: center; backdrop-filter: blur(10px);">
            ...
            <p style="color: #ffffff; font-size: 18px; margin-bottom: 32px; line-height: 1.5;">
              Clicca il bottone qui sotto per accedere istantaneamente alla community streetwear più autentica
            </p>
            ...
            <div style="margin-top: 40px; padding-top: 24px; border-top: 1px solid rgba(255,255,255,0.1);">
              <p style="color: #9ca3af; font-size: 14px; margin: 8px 0;">
                <strong>Questo link è valido per 10 minuti</strong>
              </p>
              <p style="color: #6b7280; font-size: 12px;">
                Se non hai richiesto l'accesso, puoi ignorare questa email.<br>
      ...
      const info = await transporter.sendMail({
        ...
        to: email,
        subject: '🔗 Magic Link - Accedi al tuo account',
...
// Invia magic link email (legacy function)
export async function sendMagicLinkEmail(email: string, token: string, baseURL: string) {
  ...
  console.log('🔧 SendMagicLinkEmail called with:');
  console.log(`  📧 Email: ${email}`);
...
export function isTokenExpired(createdAt: Date, expiresInMinutes: number = 10): boolean {
  const now = new Date();
